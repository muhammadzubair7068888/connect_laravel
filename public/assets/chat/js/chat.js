...
!function() {
/*!******************************************!*\
  !*** ./resources/assets/chat/js/chat.js ***!
  \******************************************/
...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        if (data.success && latestSelectedUser === groupOrUserObj.id) {
          ...
          selectedContactImg = groupOrUserObj.avatar;
          ...
          if ($('.chat__people-body').find('.chat__person-box--active').length <= 0) {
            ...
            if (!isUserElePresent) {
              var newUserEle = prepareNewConversation(groupOrUserObj.id, groupOrUserObj.name, '', groupOrUserObj.avatar, groupOrUserObj.is_online, 0);
  ...
  function newMessageArrived(e) {
    ...
    if ($.inArray(selectedContactId, [e.from_id, e.to_id]) >= 0) {
      ...
      var newUserEle = prepareNewConversation(e.from_id, htmlSpecialCharsDecode(e.sender.name), e, e.sender.avatar);
      chatPeopleBodyEle.prepend(newUserEle);
  ...
  $(document).on('click', '.user-list-chat-select__list-item', function () {
    ...
    startNewConversation(); // $('.modal-backdrop .fade').removeClass('modal-backdrop');
  });
  ...
  function prepareContactForModal(users) {
  ...
  function searchEleSearchEvent(searchEle) {
  ...
  window.deleteMsgForEveryone = function (messageId, previousMessageId) {
    $.ajax({
      type: 'post',
      url: '/chat/conversations/' + messageId + '/delete',
      data: {
  ...
  Echo["private"]("updates").listen('UpdatesEvent', function (e) {
    if (e.type == 1) {
      ...
      var user = e.user;
      $('#userListForChat').find('.chat-user-' + user.id).find('img').attr('src', user.avatar);
      $('#userListForChat').find('.chat-user-' + user.id).find('.add-user-contact-name').text(user.name);
      $('#user-' + user.id).find('img').attr('src', user.avatar);
      $('#user-' + user.id).find('.contact-name').text(user.name);
      $('.user-chat-image-' + user.id).attr('src', user.avatar).attr('data-original-title', user.name);
      var groupMember = $('#nav-group-members').find('#user-' + user.id);
      groupMember.find('.contact-name').text(user.name);
      groupMember.find('img').attr('src', user.avatar);
    } else if (e.type == 2) {
      //set user status
  ...
  function prepareNewGroupChatConversation(groupObj) {
    ...
    var newUserEle = prepareNewConversation(groupObj.id, htmlSpecialCharsDecode(groupObj.name), '', groupObj.avatar, '', 1);
    chatPeopleBodyEle.prepend(newUserEle);
    $('#user-' + groupObj.id).find('.chat-message').text(msgText);
    $('#user-' + groupObj.id).find('.chat__person-box-count').removeClass('d-none').text(1);
    $('#user-' + groupObj.id).find('.chat__person-box-time').text(getLocalDate(groupObj.created_at));
  ...
  function groupMessageArrived(e) {
    if (selectedContactId == e.to_id) {
      ...
      var newUserEle = prepareNewConversation(e.to_id, htmlSpecialCharsDecode(e.group.name), e, e.group.avatar, '', 1);
      chatPeopleBodyEle.prepend(newUserEle);
      noConversationEle.hide();
    }
  }
  ...
  function updateGroupDetails(group) {
    ...
    $('#groupDetailsImage-' + group.id).attr('src', group.avatar);
    $('#user-' + group.id).find('.chat__person-box-name').text(group.name);
    $('#user-' + group.id).find('.chat__person-box-avtar img').attr('src', group.avatar);
    $('.group-list-' + group.id).find('img').attr('src', group.avatar);
    ...
    $('.chat-header-img').attr('src', group.avatar);
  ...
  function performActionAfterUpdatingGroup(group) {
    ...
    $('#groupDetailsImage-' + groupId).attr('src', group.avatar);
    $('.group-list-' + groupId).find('img').attr('src', group.avatar);
    ...
    $('.chat__person-box--active').find('.chat__person-box-avtar img').attr('src', group.avatar);
    ...
    $.ajax({
      ...
      success: function success(result) {
        if (result.success) {
          ...
          $('#groupPhotoPreview').attr('src', group.avatar);
